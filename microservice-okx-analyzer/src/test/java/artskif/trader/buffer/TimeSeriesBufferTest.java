...
class TimeSeriesBufferTest {
    ...
    @Test
    void removeNewerThanFirstGapNoOrSmallData() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");

        assertEquals(0, buffer.removeNewerThanFirstGap());
    ...
    @Test
    void removeNewerThanFirstGapNoGap() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");
        ...
        int removed = buffer.removeNewerThanFirstGap();
        assertEquals(0, removed);
        assertEquals(5, buffer.size());
    }

    @Test
    void removeNewerThanFirstGapSingleGapInMiddle() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");

        buffer.putItem(bucket(0), 0);
        buffer.putItem(bucket(1), 1);
        buffer.putItem(bucket(3), 3);
        buffer.putItem(bucket(4), 4);
        ...
        assertEquals(bucket(0), buffer.getFirstBucket());
        assertEquals(bucket(1), buffer.getLastBucket());
        assertEquals(1, buffer.getVersion().get());
    }

    @Test
    void removeNewerThanFirstGapGapAtStart() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");
    ...
    @Test
    void removeNewerThanFirstGapGapAtEnd() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");
        ...
        assertEquals(1, removed);
        assertEquals(3, buffer.size());
        assertFalse(buffer.getDataMap().containsKey(bucket(4)));
        assertEquals(bucket(0), buffer.getFirstBucket());
        assertEquals(bucket(2), buffer.getLastBucket());
    }

    @Test
    void removeOlderThanLastGapNoOrSmallData() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");

        assertEquals(0, buffer.removeOlderThanLastGap());
    ...
    @Test
    void removeOlderThanLastGapNoGap() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");
        ...
        int removed = buffer.removeOlderThanLastGap();
        assertEquals(0, removed);
        assertEquals(5, buffer.size());
    }

    @Test
    void removeOlderThanLastGapSingleGapInMiddle() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");
        ...
        // ожидаемое фактическое поведение по реализации:
        // найден gap между bucket(1) и bucket(3) при обходе с конца, gapBucket = bucket(1)
        // удаляются все элементы <= bucket(1) -> остаются bucket(3), bucket(4)
        assertEquals(2, removed);
        assertEquals(2, buffer.size());
        assertTrue(buffer.getDataMap().containsKey(bucket(3)));
        assertTrue(buffer.getDataMap().containsKey(bucket(4)));
        assertEquals(bucket(3), buffer.getFirstBucket());
        assertEquals(bucket(4), buffer.getLastBucket());
        assertEquals(1, buffer.getVersion().get());
    }

    @Test
    void removeOlderThanLastGapMultipleGapsTakesFirstFromEnd() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");
        ...
        // первый найденный с конца разрыв между bucket(2) и bucket(4)
        // gapBucket = bucket(2), удаляются все <= bucket(2) -> остаётся только bucket(4)
        assertEquals(2, removed);
        assertEquals(1, buffer.size());
        assertTrue(buffer.getDataMap().containsKey(bucket(4)));
        assertEquals(bucket(4), buffer.getFirstBucket());
        assertEquals(bucket(4), buffer.getLastBucket());
    }

    @Test
    void removeOlderThanLastGapGapAtStart() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");
        ...
        // обход с конца обнаружит gap между bucket(0) и bucket(2), gapBucket = bucket(0)
        // удаляется только bucket(0), остаётся bucket(2)
        assertEquals(1, removed);
        assertEquals(1, buffer.size());
        assertTrue(buffer.getDataMap().containsKey(bucket(2)));
        assertEquals(bucket(2), buffer.getFirstBucket());
    ...
    @Test
    void removeOlderThanLastGapGapAtEnd() {
        TimeSeriesBuffer<Integer> buffer = new TimeSeriesBuffer<>(10, BUCKET, "test");
        ...
        // gap между bucket(1) и bucket(3), gapBucket = bucket(1)
        // удаляются bucket(0) и bucket(1), остаётся bucket(3)
        assertEquals(2, removed);
        assertEquals(1, buffer.size());
        assertTrue(buffer.getDataMap().containsKey(bucket(3)));
        assertEquals(bucket(3), buffer.getFirstBucket());
